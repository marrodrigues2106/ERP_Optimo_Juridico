cronAdd('pje_health_check', '*/15 * * * *', () => {
  ...
  try {
    ...
    if (pjeRes.statusCode >= 200 && pjeRes.statusCode < 400) pjeOnline = true
    else throw new Error(`HTTP ${pjeRes.statusCode}`)
  } catch (err) {
    try {
  ...
  try {
    ...
    if (djRes.statusCode >= 200 && djRes.statusCode < 400) datajudOnline = true
    else throw new Error(`HTTP ${djRes.statusCode}`)
  } catch (err) {
    try {
      const logCol = $app.findCollectionByNameOrId('system_logs')
      const log = new Record(logCol)
  ...
  try {
    ...
    if (scrapeRes.statusCode === 200) scrapingConnected = true
    else throw new Error(`HTTP ${scrapeRes.statusCode}`)
  } catch (err) {
    try {
      const logCol = $app.findCollectionByNameOrId('system_logs')
      const log = new Record(logCol)
  ...
  if (!datajudOnline) config.set('datajudLastError', 'Connection failed')
  else config.set('datajudLastError', '')
